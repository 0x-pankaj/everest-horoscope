...
const OurServices: React.FC = () => {
  ...
  const nextServices = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex + servicesPerPage.mobile < services.length
        ? prevIndex + servicesPerPage.mobile
        : prevIndex,
    );
  };
  ...
  const prevServices = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex - servicesPerPage.mobile >= 0
        ? prevIndex - servicesPerPage.mobile
        : prevIndex,
    );
  };
  ...
  const visibleServices = services.slice(
    currentIndex,
    currentIndex + servicesPerPage.desktop,
  );
  ...
  return (
    <div className="bg-gradient-to-r from-yellow-100 to-orange-100 p-2 md:p-6">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-md p-3 md:p-6">
        <h2 className="text-xl md:text-3xl font-bold mb-3 md:mb-6 text-center text-yellow-800">
          Our Services
        </h2>
        {services.length === 0 ? (
          ...
          <>
            <div className="flex justify-between items-center mb-3 md:mb-4">
              ...
              <span className="text-sm md:text-base text-yellow-600">
                {currentIndex + 1}-
                {Math.min(
                  currentIndex + servicesPerPage.mobile,
                  services.length,
                )}{" "}
                of {services.length}
              </span>
              {services.length > 8 ? (
                <button
                  ...
                  disabled={
                    currentIndex + servicesPerPage.mobile >= services.length
                  }
                  className="bg-yellow-500 text-white px-2 py-1 md:px-4 md:py-2 rounded-full disabled:opacity-50 transition-colors duration-300"
                >
                  <FaChevronRight />
              ...
              {visibleServices.map((service, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-yellow-800 bg-opacity-70 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <p className="text-yellow-100 text-center font-semibold px-2 text-xs md:text-sm">
                      {service.name}
                    </p>
                  </div>
                </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-4 md:p-6 max-w-md w-full">
        <h2 className="text-xl md:text-2xl font-bold mb-2 md:mb-4 text-yellow-800">
          {service.name}
        </h2>
