...
const VastoForm = () => {
  ...
  const [formData, setFormData] = useState<
    Omit<VastoFormData, "houseMap"> & { houseMap: File | null }
  >({
    id: "",
    name: "",
    email: "",
  ...
  const handleServiceToggle = (service: string) => {
    setFormData((prev) => {
      const currentServices = prev.selectedServices || []; // Use empty array if undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("userId before subimiting : ", user?.$id);
      ...
      // Handle the services array // Only append selectedServices if there are any
      const services = formData.selectedServices ?? [];
      if (services.length > 0) {
        submitFormData.append("selectedServices", JSON.stringify(services));
      }
