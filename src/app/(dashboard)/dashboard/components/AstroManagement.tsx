...
export default function AstroManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = isEditing ? { ...currentAstrologer, $id: (currentAstrologer as Astrologer).$id } : currentAstrologer;
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-yellow-50">
      <h1 className="text-3xl font-bold mb-8 text-center text-yellow-800">Astrologer Management</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {astrologers.map((astro) => (
          <div key={astro.$id} className="bg-white shadow-lg rounded-lg p-6 border border-yellow-200">
            <div className="flex items-center mb-4">
              <img src={astro.photoUrl} alt={astro.name} className="w-16 h-16 rounded-full mr-4" />
              <div>
                <h2 className="text-xl font-semibold text-yellow-800">{astro.name}</h2>
                <p className="text-yellow-600">Email: {astro.email}</p>
              </div>
            </div>
            <p className="text-yellow-700 mb-2">{astro.bio}</p>
            <p className="text-yellow-700 mb-2">Specialties: {astro.specialties.join(', ')}</p>
            <p className="text-yellow-700 mb-2">Rating: {astro.rating}</p>
            <p className="text-yellow-700 mb-2">Experience: {astro.experience} years</p>
            <p className="text-yellow-700 mb-2">Hourly Rate: ${astro.hourlyRate}</p>
            <p className="text-yellow-700 mb-2">Status: {astro.isOnline ? 'Online' : 'Offline'}</p>
            {astro.language && <p className="text-yellow-700 mb-4">Languages: {astro.language.join(', ')}</p>}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h3 className="text-lg font-semibold mb-4 text-yellow-800">{isEditing ? 'Edit Astrologer' : 'Add New Astrologer'}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="photoUrl" className="block text-sm font-medium text-yellow-700">Photo URL</label>
                <input
                  id="photoUrl"
                  name="photoUrl"
                  type="text"
              ...
              <div>
                <label htmlFor="bio" className="block text-sm font-medium text-yellow-700">Bio</label>
                <textarea
                  id="bio"
                  name="bio"
              ...
              <div>
                <label htmlFor="specialties" className="block text-sm font-medium text-yellow-700">Specialties (comma separated)</label>
                <input
                  id="specialties"
                  name="specialties"
                  type="text"
              ...
              <div>
                <label htmlFor="rating" className="block text-sm font-medium text-yellow-700">Rating</label>
                <input
                  id="rating"
                  name="rating"
                  type="number"
              ...
              <div>
                <label htmlFor="experience" className="block text-sm font-medium text-yellow-700">Experience (years)</label>
                <input
                  id="experience"
                  name="experience"
                  type="number"
              ...
              <div>
                <label htmlFor="hourlyRate" className="block text-sm font-medium text-yellow-700">Hourly Rate ($)</label>
                <input
                  id="hourlyRate"
                  name="hourlyRate"
                  type="number"
              ...
              <div>
                <label htmlFor="isOnline" className="block text-sm font-medium text-yellow-700">Is Online?</label>
                <input
                  id="isOnline"
                  name="isOnline"
                  type="checkbox"
              ...
              <div>
                <label htmlFor="language" className="block text-sm font-medium text-yellow-700">Languages (comma separated)</label>
                <input
                  id="language"
                  name="language"
                  type="text"
      ...
      {deleteConfirmationId && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <h3 className="text-lg font-semibold mb-4 text-yellow-800">Delete Confirmation</h3>
            <p className="text-yellow-700">Are you sure you want to delete this astrologer?</p>
            <div className="mt-4 flex justify-end">
              <button
