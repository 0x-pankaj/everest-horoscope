...
export async function POST(request: Request) {   
  try {
    ...
    if (existingAstrologers.total > 0) {
      return NextResponse.json({ error: 'An astrologer with this email already exists' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: `Failed to create astrologer: ${error.message} ` }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
    try {
      ...
      return NextResponse.json({ error: `Failed to update astrologer: ${error.message}` }, { status: 500 });
    }
  }
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete astrologer' }, { status: 500 });
  }
}